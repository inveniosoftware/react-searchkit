...
describe('test InvenioRecordsResourcesRequestSerializer serializer', () => {
  const serializer = new InvenioRecordsResourcesRequestSerializer();
  ...
  it('should serialize the multiple filters', () => {
    const queryState = {
      ...
      page: 0,
      size: 0,
      filters: [['type', 'report'], ['category', 'video']],
    };
    const strState = serializer.serialize(queryState);
  ...
  it('should serialize nested filters', () => {
    const queryState = {
      ...
      page: 0,
      size: 0,
      filters: [['type', 'report', ['subtype', 'pdf']], ['category', 'video']],
    };
    const strState = serializer.serialize(queryState);
  ...
  it('should fail when filters have wrong format', () => {
    [
      [['wrong=format']],
      [['type', 'report', 'wrong=format'], ['category', 'video']],
